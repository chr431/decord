...
**口径声明（重要）**：下表是**纯解码**数字——只测 decord 解码本身，不含任何
引擎侧耦合（无推理、无分段、无消费线程）。引擎 e2e 数字（含 OCR 等）在
[chr431/video_ocr_engine](https://github.com/chr431/video_ocr_engine) 仓，
**两者不可混用**。本表可由本仓 `tests/bench_decode_only.py` 一键复现。

本机实测（AMD Ryzen 9 7945HX + RTX 4060 Laptop，Windows，FFmpeg 9.0 shared，
1080p，`output_format='yuv420'`，**全帧**，`get_batch` 每批 250 帧顺序读，
全片 23970 帧 × 5 轮取中位，单位 fps；2026-09-19 `v0.8.4` 口径）。
不同机器/码流/分辨率/**电源计划**差异很大，请自行实测；hybrid 属实验性接口。

| 解码方式 | H.264 | HEVC | AV1 |
|---|---|---|---|
| `cpu`（解码线程：h264/hevc 32、av1 24） | 1386 | 841 | 996 |
| `gpu`（NVDEC，全帧） | 864 | 1782 | 1511 |
| `hybrid_gpu`（实验性，→显存） | **1484** | **1978** | **2074** |

要点：

- **hybrid 在三个编码上都快于两个单后端**：相对最优单臂 +7%（h264，vs cpu）、
  +11%（hevc，vs NVDEC）、+37%（av1，vs NVDEC）。h264 的领先幅度最小，因为它
  的 CPU 软解本身就很快（1386fps），hybrid 需要两臂并行才有增益。
- **解码速率是码流相关的，不要按编码格式推断**：同为 H.264，软编（x264 medium
  加权预测）码流的 CPU 软解可比硬编风格码流慢 1.6×——本表 h264 数字来自
  x264 medium 码流。选型应按实测速率而非编码名。
